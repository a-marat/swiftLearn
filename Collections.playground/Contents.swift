...
var dictionaryEmpty = [Int: String]() // Инициализация пустого словаря с целочисленным ключом и строковым значением

var results = ["Mathematic": 80, "Physics": 70, "English": 65]

results.count // Подсчет количества элементов
results.isEmpty // Проверка на наличие элементов

results["Geometric"] = 55 // Добавление нового элемента
results["English"] = 75 // Изменение значения элемента

results.updateValue(87, forKey: "Physics") // Обновление значения ключа с возвращением предыдущего значения
results.removeValue(forKey: "English") // Удаление элемента с возвращением ключа

results["Physics"] = nil // Удаление пары ключ-значение без возврата значения

for (subject, point) in results {
    print("Предмет \(subject) баллы \(point)")
}

for subjects in results.keys {
    print("Предмет \(subjects)")
}
...
let subjectName = Array(results.keys) // Создание массива с присовением ему ключей из словаря results
print(subjectName)

let pointValue = Array(results.values) // Создание массива с присвоением ему значений из словаря results
print(pointValue)
